...
// returns the sum of these numbers. Run the previous program and see whether it does indeed return 55.

// As a bonus assignment, modify your range function to take an
// optional third argument that indicates the “step” value used to
// build up the array. If no step is given, the array elements go up by
...
function range(start,end,step) {
  ...
  if (start < end) {
    for (var i = start; i <= end; increment_up()) {
      ary.push(i);
    }
    return ary;
  } else {
    for (var x = start; x >= end; increment_down()) {
      ary.push(x);
    }
    return ary;
  }
...
// the previous chapter, which variant do you expect to be useful in more situations?
...
// they are both independent lists, but they share the structure that makes up their
...
// array from a list. Also write the helper functions prepend, which takes an element and a
// list and creates a new list that adds the element to the front of the input list, and nth,
...
// the same value or are objects with the same properties whose values are also equal when
